...
for n in range(1, WAVNUM):
    ...
    # FFT Process
    eval('S'+str(n)).y = FFT_Process(eval('S'+str(n)).d, eval('S'+str(n)).f, eval('S'+str(n)).t, eval('S'+str(n)).im, 8, 12)

    # Data Process
    DATA_Process(eval('S'+str(n)).y, eval('S'+str(n)).t, WAVFile[n], Grouping, "Alpha")
...
# Plot Section
plt.plot(S1.t, S1.y, label='S1_B1', color=GraphColor[0])
plt.plot(S2.t, S2.y, label='S1_B2', color=GraphColor[1])
plt.plot(S3.t, S3.y, label='S1_B3', color=GraphColor[2])
plt.plot(S4.t, S4.y, label='S2_B1', color=GraphColor[3])
plt.plot(S5.t, S5.y, label='S2_B2', color=GraphColor[4])
plt.plot(S6.t, S6.y, label='S2_B3', color=GraphColor[5])
plt.plot(S7.t, S7.y, label='S3_B1', color=GraphColor[6])
